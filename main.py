...
def get_user_plan(user_id):
    ...
    if user_id == OWNER_ID and OWNER_ID > 0 and OWNER_FREE_UNLIMITED:
        return {
            "plan": PLAN_UNLIMITED,
            "uses_remaining": -1,
...
def can_use_bulk(user_id):
    if user_id == OWNER_ID and OWNER_ID > 0 and OWNER_FREE_UNLIMITED:
        return True, "👑 𝙊𝙒𝙉𝙀𝙍 𝙁𝙍𝙀𝙀 | ♾️ 𝙐𝙉𝙇𝙄𝙈𝙄𝙏𝙀𝘿"
    plan = get_user_plan(user_id)
...
async def notify_owner_payment(update, context, plan_name, stars, payment):
    """Notify OWNER_ID after a successful Telegram Stars purchase."""
    try:
        ...
        logging.getLogger("PAYMENT").warning(
            f"Failed to notify owner about payment: {e}"
        )
