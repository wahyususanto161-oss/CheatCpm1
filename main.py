...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock clothing using CPM's native flat equipment representation."""
        await self.load(uid, force=True)
        td = self.get_token_data(uid) or {}
        email = td.get("email")
        d = deepcopy(self.get_record(uid, email))
        ...
        def normalize(items):
            ...
            if isinstance(items, list):
                for item in items:
                    ...
                    if key not in seen:
                        seen.add(key)
                        existing.append({"type": typ, "id": iid, "color": color})

            # Preserve existing items and add the supported clothing IDs.
        ...
        d["personEquipmentsMale"] = normalize(d.get("personEquipmentsMale"))
        d["personEquipmentsFemale"] = normalize(d.get("personEquipmentsFemale"))

        # Use the native full-record serializer, then verify with a fresh load.
        saved = await self._full_save_record(uid, d)
        if not saved.get("ok"):
            return saved
        ...
        def verified(fresh):
            for key in ("personEquipmentsMale", "personEquipmentsFemale"):
                ...
                if not isinstance(items, list):
                    return False
                ids = {int(x.get("id", -1)) for x in items if isinstance(x, dict) and int(x.get("type", 0)) == 0}
                if not set(range(1, 200)).issubset(ids):
                    return False
            return True

        result = await self._reload_and_verify(uid, verified, "ALL_CLOTHES")
        if result.get("ok"):
