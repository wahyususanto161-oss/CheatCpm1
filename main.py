...
class Writer:
    ...
    def write_equipment_flat(self, data):
        """Serialize CPM equipment in the native flat type/id/color format."""
        items = data or []
...
def equipment_dict_to_flat(data):
    """Convert main.py's category cache to the flat CPM equipment wire format."""
    if isinstance(data, list):
        return data
    if not isinstance(data, dict):
        return []

    categories = ("hair", "face", "beard", "cap", "mask", "top",
...
def serialize_field(fid, value):
    ...
    if fid in (41,42):
        # Field 41/42 uses CPM's flat equipment wire format.
        w.write_equipment_flat(equipment_dict_to_flat(value))
        return w.to_bytes()
...
def serialize_full_record(record):
    ...
    w.write_equipment_flat(equipment_dict_to_flat(record.get("personEquipmentsMale")))
    w.write_equipment_flat(equipment_dict_to_flat(record.get("personEquipmentsFemale")))
...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock clothing in the native main.py equipment structure and verify it."""
        await self.load(uid, force=True)
        td = self.get_token_data(uid) or {}
        email = td.get("email")
        d = deepcopy(self.get_record(uid, email))
        ...
        # IMPORTANT:
        # main.py's Reader/Writer represent personEquipmentsMale/Female as
        # dictionaries of clothing categories. Do NOT replace these dictionaries
        # with the flat type/id/color list used by mainbotcpm.py.
        categories = (
        ...
        def normalize_equipment(equip, gender):
            ...
            for category in categories:
                ...
                # Only integer item IDs belong in these category lists.
                cleaned = set()
                ...
                # Keep existing inventory and add the supported clothing range.
                # The previous implementation used 1..300, which could make the
                # server reject the complete record. Use the same 1..199 range
                # requested by the original feature mechanism.
                cleaned.update(range(1, 200))
                out[category] = sorted(cleaned)

            # These fields describe equipped state/gender, not owned inventory.
        ...
        d["personEquipmentsMale"] = normalize_equipment(
            d.get("personEquipmentsMale"), 0
        )
        d["personEquipmentsFemale"] = normalize_equipment(
            d.get("personEquipmentsFemale"), 1
        )

        log.info(
            "All Clothes payload: male=%d female=%d",
            len(equipment_dict_to_flat(d["personEquipmentsMale"])),
            len(equipment_dict_to_flat(d["personEquipmentsFemale"])),
        )

        # The partial-save endpoint receives flat equipment records through
        # serialize_field(); the local record remains category-based.
        # This fixes the "'list' object has no attribute 'get'" error from V3.
        saved = await self._full_save_record(uid, d)

        if not saved.get("ok"):
            # Try the authenticated partial-save path as a fallback.
            saved = await self._save(uid, d)
        ...
        verified = await self._reload_and_verify(
            uid, clothes_verified, "ALL_CLOTHES"
        )
        ...
        return {
            "ok": True,
            "message": "All Clothes saved and verified from server."
        }
...
async def cb_s_email(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(hdr("📧","𝗖𝗛𝗔𝗡𝗚𝗘 𝗘𝗠𝗔𝗜𝗟")+"\n\n  Enter new CPM email:", reply_markup=K.cancel())
    await cb.answer()
