...
class Writer:
    ...
    def write_equipment_flat(self, data):
        """Serialize equipment for the partial-save wire format."""
        items = data or []
...
def equipment_dict_to_flat(data):
    """Convert local category inventory to CPM partial-save flat equipment records."""
...
class CPMNuker:
    ...
    async def unlock_headlights(self, uid):
        """Unlock headlights using the next feature flag in the float table."""
        return await self._set_floats(uid, [(35, 1.0)])

    async def unlock_all_clothes(self, uid):
        """Add a broad set of clothing IDs to both male/female inventories."""
        await self.load(uid)
        td = self.get_token_data(uid)
        email = td.get("email") if td else None
        d = deepcopy(self.get_record(uid, email))
        ...
        # Keep the currently owned items and add clothing IDs.
        # SelectedEquipments is intentionally left untouched because it
        # represents currently equipped items, not the owned inventory.
        clothing_ids = list(range(1, 200))

        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            equip = deepcopy(d.get(key) or {})
            if not equip:
                equip = {
                    "hair": [], "face": [], "beard": [], "cap": [], "mask": [],
                    "top": [], "gloves": [], "bag": [], "pants": [], "shoes": [],
                    "glasses": [], "SelectedEquipments": [], "Gender": 0
                }
            for part in ("hair", "face", "beard", "cap", "mask", "top",
                         "gloves", "bag", "pants", "shoes", "glasses"):
                current = equip.get(part) or []
                equip[part] = sorted(set(current) | set(clothing_ids))
            d[key] = equip
