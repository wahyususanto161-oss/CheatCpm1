...
BOT_TOKEN = "8734052502:AAGtlLtlI4bZdMoDZg09NEeTikXSTCyBPdY"
OWNER_ID  = 8700382637
...
# ═══════════════════════════════════════════
#  💡 HEADLIGHT COLOR PERSISTENCE
# ═══════════════════════════════════════════

HEADLIGHT_COLOR_KEYWORDS = (
    "headlightcolor", "headlightscolor", "headlight_color", "headlights_color",
    "headlightcolour", "headlightscolour", "headlight_colour", "headlights_colour",
    "lightcolor", "light_color", "frontlightcolor", "front_light_color",
    "colorheadlight", "colorheadlights",
)

def _normalize_headlight_hex(value):
    value = str(value or "").strip().upper()
    if value.startswith("#"):
        value = value[1:]
    if not re.fullmatch(r"[0-9A-F]{6}(?:[0-9A-F]{2})?", value):
        raise ValueError("COLOR_FORMAT_INVALID")
    return "#" + value

def _hex_rgb(value):
    h = _normalize_headlight_hex(value)[1:]
    return int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16)

def _color_for_existing_shape(new_hex, old):
    r, g, b = _hex_rgb(new_hex)

    if isinstance(old, str):
        return new_hex if old.strip().startswith("#") else new_hex[1:]
    if isinstance(old, int) and not isinstance(old, bool):
        return (r << 16) | (g << 8) | b
    if isinstance(old, float):
        return float((r << 16) | (g << 8) | b)

    if isinstance(old, list) and len(old) in (3, 4):
        normalized = all(
            isinstance(x, (int, float)) and 0 <= float(x) <= 1.0
            for x in old[:3]
        )
        vals = [r, g, b]
        if normalized:
    ...
    if isinstance(old, dict):
        lower = {str(k).lower(): k for k in old.keys()}
        rgb_keys = None
        for names in (("r", "g", "b"), ("red", "green", "blue")):
            if all(n in lower for n in names):
                rgb_keys = tuple(lower[n] for n in names)
                break
        if rgb_keys:
            out = deepcopy(old)
            normalized = all(
                isinstance(out[k], (int, float)) and 0 <= float(out[k]) <= 1.0
                for k in rgb_keys
            )
            vals = (
                (r / 255.0, g / 255.0, b / 255.0)
                if normalized else (r, g, b)
            )
            for k, v in zip(rgb_keys, vals):
                out[k] = v
            return out

    raise ValueError("HEADLIGHT_COLOR_SHAPE_UNKNOWN")

def _find_headlight_color_slots(obj, path=()):
    found = []
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if any(token in norm for token in HEADLIGHT_COLOR_KEYWORDS):
                found.append((path, key, value))
            elif "headlight" in norm and isinstance(value, dict):
                for child_key, child_value in value.items():
                    child_norm = re.sub(r"[^a-z0-9]", "", str(child_key).lower())
                    if child_norm in {
                        "color", "colour", "rgb", "colorvalue", "colourvalue"
                    }:
                        found.append((path + (key,), child_key, child_value))

            found.extend(_find_headlight_color_slots(value, path + (key,)))

    elif isinstance(obj, list):
        for i, value in enumerate(obj):
            found.extend(_find_headlight_color_slots(value, path + (i,)))

    return found

def _decode_all_data_json(text):
...
def _encode_all_data_json(value):
    return json.dumps(value, ensure_ascii=False, separators=(",", ":"))

def _set_nested_path(obj, path, key, value):
    cur = obj
    for part in path:
        cur = cur[part]
    cur[key] = value

def _get_nested_path(obj, path):
    cur = obj
    for part in path:
        cur = cur[part]
    return cur

def set_headlight_color_in_record(record, color_hex):
    """
    Update an EXISTING Headlight Color field in allData.

    We deliberately do not invent a new schema or guess a float index.
    The account record must expose a recognizable Headlight Color field.
    """
    color_hex = _normalize_headlight_hex(color_hex)

    decoded = _decode_all_data_json(record.get("allData"))
    if decoded is None:
        return {
            "ok": False,
            "message": "HEADLIGHT_COLOR_SCHEMA_NOT_JSON",
        }

    slots = _find_headlight_color_slots(decoded)
    if not slots:
        return {
            "ok": False,
            "message": "HEADLIGHT_COLOR_SCHEMA_NOT_FOUND",
        }

    # Prefer a direct headlight-color key.
    path, key, old = slots[0]

    try:
        new_value = _color_for_existing_shape(color_hex, old)
    except ValueError as e:
        return {"ok": False, "message": str(e)}

    updated = deepcopy(decoded)
    _set_nested_path(updated, path, key, new_value)
    record["allData"] = _encode_all_data_json(updated)
...
def get_headlight_color_from_record(record, path):
    try:
        decoded = _decode_all_data_json(record.get("allData"))
        if decoded is None or not path:
            return None
        parent = _get_nested_path(decoded, tuple(path[:-1]))
        return parent[path[-1]]
    except Exception:
        return None

def find_headlight_color_candidates(record):
    decoded = _decode_all_data_json(record.get("allData"))
    if decoded is None:
        return []
    return [
        {
            "path": ".".join(str(x) for x in (list(path) + [key])),
            "value": value,
        }
        for path, key, value in _find_headlight_color_slots(decoded)
    ]
...
class CPMNuker:
    ...
    async def _save(self, uid, data):
        """
        Safe save that avoids overwriting changes made by the CPM APK after
        the bot's last load.

        In particular, Headlight Color is not decoded by this bot. It may live
        in a part of the account record that this bot does not expose directly.
        Therefore, before every save we refresh the server record and apply
        only the fields that the bot actually changed.

        For the floats list we merge per-index so an unrelated Headlight Color
        value in another float slot is preserved.
        """
        ok,msg,auth = await self.get_auth(uid)
        if not ok:
            return {"ok":False,"message":msg}
        ...
        cached = deepcopy(self.get_record(uid,email) or {})
        if not cached:
            cached = {}

        # Refresh the newest server-side account state. This is important when
        # the user changed Headlight Color directly inside CPM after the bot
        # last loaded the account.
        fresh_ok = await self.load(uid, force=True)
        latest = deepcopy(self.get_record(uid,email) or cached)
        ...
        if cached and latest:
            ...
            for key, new_value in data.items():
                ...
                if key == "floats" and isinstance(new_value, list) and isinstance(old_value, list):
                    # Merge only float indexes changed by the bot. This prevents
                    # a stale cached float from resetting another float changed
                    # by the CPM APK (including a possible Headlight Color slot).
                    remote = list(latest.get("floats") or [])
                    n = max(len(new_value), len(old_value))
                    while len(remote) < n:
                        remote.append(0.0)

                    for i in range(n):
                        ov = old_value[i] if i < len(old_value) else None
                        nv = new_value[i] if i < len(new_value) else None
                        if nv is not None and _field_modified(nv, ov):
                            remote[i] = nv

                    merged["floats"] = remote
                    continue

                if _field_modified(new_value, old_value):
                    merged[key] = deepcopy(new_value)

            data_to_save = merged
            original_for_payload = latest
        else:
            data_to_save = deepcopy(data)
            original_for_payload = latest or None

        ok2,msg2 = await self._send(auth,data_to_save,fuid,original_for_payload)
        if ok2:
            self.set_record(uid,data_to_save,email)
            STORE["stats"]["total_actions"] = STORE["stats"].get("total_actions",0)+1
            save_store(STORE); update_daily_stats("actions")
            return {
                "ok":True,
                "headlight_preserved": bool(fresh_ok),
                "message": "Saved without overwriting the latest CPM server state."
            }
    ...
    async def set_headlight_color(self, uid, color_hex):
        """
        Persist a Headlight Color through the account record when the actual
        field is present in allData. The server record is refreshed first and
        the exact changed slot is verified after saving.
        """
        try:
            color_hex = _normalize_headlight_hex(color_hex)
        except ValueError as e:
        ...
        if not await self.load(uid, force=True):
            return {"ok": False, "message": "HEADLIGHT_COLOR_LOAD_FAILED"}
        ...
        if len(floats) <= 35 or float(floats[35]) < 1.0:
            return {"ok": False, "message": "HEADLIGHTS_NOT_UNLOCKED"}

        changed = set_headlight_color_in_record(current, color_hex)
        ...
        if not await self.load(uid, force=True):
            return {"ok": False, "message": "HEADLIGHT_COLOR_VERIFY_LOAD_FAILED"}

        fresh = self.get_record(uid, email) or {}
        verified = get_headlight_color_from_record(fresh, path)

        if verified != changed["stored_value"]:
            return {
                "ok": False,
                "message": "HEADLIGHT_COLOR_VERIFY_FAILED",
                "expected": changed["stored_value"],
                "received": verified,
        ...
        return {
            "ok": True,
            "message": "Headlight color saved and verified.",
            "color": color_hex,
            "path": ".".join(map(str, path)),
        }

    async def inspect_headlight_color(self, uid):
        """Show whether the current account exposes a recognizable color field."""
        if not await self.load(uid, force=True):
            return {"ok": False, "message": "HEADLIGHT_COLOR_LOAD_FAILED"}

        td = self.get_token_data(uid) or {}
        email = td.get("email")
        record = self.get_record(uid, email) or {}
        candidates = find_headlight_color_candidates(record)
    ...
    async def unlock_headlights(self, uid):
        """Enable native HEADLIGHTS only; color is changed inside the CPM APK."""
        ...
        # Only the known HEADLIGHTS feature flag is changed. Do not touch any
        # other float slots because the actual Headlight Color field is not
        # identified by this parser.
        floats = list(current.get("floats") or [])
        while len(floats) <= 35:
            floats.append(0.0)
        floats[35] = 1.0
        ...
        # Verify that the unlock persisted on the server.
        if not await self.load(uid, force=True):
            return {"ok": False, "message": "HEADLIGHTS_VERIFY_LOAD_FAILED"}
        fresh = self.get_record(uid, email) or {}
        vals = fresh.get("floats") or []
        try:
            unlocked = len(vals) > 35 and float(vals[35]) >= 1.0
        except (TypeError, ValueError):
            unlocked = False
        if not unlocked:
        ...
        return {
            "ok": True,
            "message": "Headlights unlocked. Change color directly inside the CPM APK.",
            "native_color": True,
        }
    ...
    def feat(uid=0):
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(
                text="🎨  HEADLIGHT COLOR • SET / SYNC",
                callback_data="f_headlight_color")],
            [InlineKeyboardButton(
...
class SHeadlightColor(StatesGroup):
    color = State()
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    td = nuker.get_token_data(uid)
    if td:
        # Logged-in users always get the PHOTO dashboard card.
        # This keeps the Welcome and Dashboard layouts consistent.
        await show_home(msg, uid)
    else:
        # First screen: Welcome photo + welcome caption + buttons.
...
async def cb_send_request(cb: CallbackQuery):
    ...
    if is_banned(uid):
        await safe_edit_callback(cb, T.banned()); await cb.answer(); return
    if is_allowed(uid):
...
async def cb_rq_accept(cb: CallbackQuery):
    ...
    try:
        photo = get_bot_photo()
        if photo:
            await bot.send_photo(uid, photo=photo, caption=T.welcome(name,un,uid), reply_markup=K.login())
        else:
            await bot.send_message(uid, T.welcome(name,un,uid), reply_markup=K.login())
    except Exception as e:
        log.warning(f"Welcome send to added user failed: {e}")
    try:
        await safe_edit_callback(cb, 
...
async def cb_check_status(cb: CallbackQuery):
    ...
    elif is_banned(uid):  await safe_edit_callback(cb, T.banned()); await cb.answer("🚫")
    else:
        await safe_edit_callback(cb, 
            f"{B}\n  ⏳  𝗣𝗘𝗡𝗗𝗜𝗡𝗚\n{B}\n\n  Still pending.",
            reply_markup=K.after_request())
...
async def cb_login(cb: CallbackQuery, state: FSMContext):
    ...
    if is_banned(uid):
        ...
        return
    if not is_allowed(uid):
        await bot.send_message(uid, T.no_access(), reply_markup=K.no_access())
        return
    if is_maintenance() and not has_admin(uid, "moderator"):
        await bot.send_message(uid, T.maintenance())
        return
                ...
                if photo:
                    try:
                        await bot.send_photo(
                            uid, photo=photo, caption=dashboard,
    ...
    if record and record.get("Name"):
        ...
        if photo:
            try:
                ...
                pass
            try:
                await bot.send_photo(uid, photo=photo, caption=dashboard, reply_markup=K.home(uid))
            except Exception as e:
...
@rt.callback_query(F.data == "f_headlight_color")
async def cb_headlight_color(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    if not await check_callback(cb):
        return
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True)
        return

    await state.set_state(SHeadlightColor.color)
    await safe_edit_callback(
        cb,
        hdr("🎨", "𝗛𝗘𝗔𝗗𝗟𝗜𝗚𝗛𝗧 𝗖𝗢𝗟𝗢𝗥") +
        "\n\n"
        "  Masukkan warna HEX yang ingin disimpan ke akun CPM.\n"
        "  Contoh: <code>#D86600</code>\n"
        "  Atau: <code>#00EDF6</code>\n\n"
        "  ⚠️ Bot hanya akan menulis field Headlight Color jika\n"
        "  field tersebut benar-benar ditemukan di data akun.",
        reply_markup=K.cancel()
    )
    await cb.answer()

@rt.message(SHeadlightColor.color)
async def p_headlight_color(msg: Message, state: FSMContext):
    color = (msg.text or "").strip()

    try:
        color = _normalize_headlight_hex(color)
    except ValueError:
        await msg.answer(
            "✗ Format warna tidak valid.\n"
            "Gunakan contoh <code>#D86600</code>.",
            reply_markup=K.cancel()
        )
        return

    await state.clear()
    ld = await msg.answer("⏳ Membaca data akun dan menyimpan Headlight Color...")

    r = await nuker.set_headlight_color(msg.from_user.id, color)

    if r.get("ok"):
        body = (
            f"🎨 <b>{color}</b>\n"
            f"✔ Path: <code>{r.get('path','-')}</code>\n\n"
            "Server mengembalikan nilai yang sama setelah verifikasi."
        )
    else:
        body = (
            f"✗ <code>{r.get('message','UNKNOWN_ERROR')}</code>\n\n"
            "Jika muncul <code>HEADLIGHT_COLOR_SCHEMA_NOT_FOUND</code>, "
            "berarti field warna belum terlihat oleh parser dan kita tidak "
            "boleh menebak index-nya."
        )

    await result_msg(
        ld,
        r.get("ok"),
        "𝗛𝗘𝗔𝗗𝗟𝗜𝗚𝗛𝗧 𝗖𝗢𝗟𝗢𝗥 𝗦𝗔𝗩𝗘𝗗" if r.get("ok") else "𝗛𝗘𝗔𝗗𝗟𝗜𝗚𝗛𝗧 𝗖𝗢𝗟𝗢𝗥 𝗙𝗔𝗜𝗟𝗘𝗗",
        body,
        K.feat(msg.from_user.id)
    )
...
async def cb_feat(cb: CallbackQuery):
    ...
    loading_text = (
        "  ⏳ Unlocking native HEADLIGHTS..."
        if cb.data == "f_headlights"
        else f"  ⏳ Loading account & applying {fname}..."
    )
    m = await safe_edit_callback(cb, loading_text)
    await cb.answer()
    r = await fn(uid)
    if r.get("ok"):
    ...
    if r.get("ok") and cb.data == "f_headlights":
        title = "💡 HEADLIGHTS UNLOCKED ✔"
        body = (
            "\n\n  ✅ Native HEADLIGHTS sudah dibuka.\n"
            "  🎨 Pilih warna di CPM seperti biasa.\n"
            "  🔄 Jika warna kembali ke default, gunakan menu\n"
            "     🎨 HEADLIGHT COLOR • SET / SYNC untuk menyimpannya.\n"
            "  ℹ️ Bot tidak menebak field warna yang tidak ada di record."
        )
    else:
        title = f"{fname} ✔" if r.get("ok") else f"{fname} ✗"
        body = "" if r.get("ok") else r.get("message","")
    await result_msg(m, r.get("ok"), title, body, K.feat(uid))
...
async def cb_s_clone(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_callback(cb, hdr("👥","𝗖𝗟𝗢𝗡𝗘 𝗔𝗖𝗖𝗢𝗨𝗡𝗧")+"\n\n  Enter the target account email you own:", reply_markup=K.cancel())
    await cb.answer()

@rt.message(SClone.email)
async def p_clone_email(msg: Message, state: FSMContext):
    em = (msg.text or "").strip()
    if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", em):
        await msg.answer("  ✗ Invalid target email.", reply_markup=K.cancel()); return
    await state.update_data(target_email=em)
    await state.set_state(SClone.password)
    await msg.answer("  Enter target account password:", reply_markup=K.cancel())
...
async def p_clone_password(msg: Message, state: FSMContext):
    ...
    data = await state.get_data()
    await state.clear()
    ld = await msg.answer("  ⏳ Cloning account data...")
    r = await nuker.clone_account(msg.from_user.id, data.get("target_email",""), pw)
    try: await msg.delete()
    except: pass
    await result_msg(ld, r.get("ok"), "𝗖𝗟𝗢𝗡𝗘 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘" if r.get("ok") else "𝗖𝗟𝗢𝗡𝗘 𝗙𝗔𝗜𝗟𝗘𝗗",
        "✔ Target account updated" if r.get("ok") else r.get("message",""), K.back_home())
