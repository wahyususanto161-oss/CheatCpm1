...
def serialize_full_record(record):
    ...
    w.write_equipment_flat(record.get("personEquipmentsMale", []))
    w.write_equipment_flat(record.get("personEquipmentsFemale", []))
    w.write_plates(record.get("platesData", None))
    w.write_car_id_status(record.get("carIDnStatus", None))
    w.write_string(record.get("allData", ""))
    w.write_dict(record.get("flags", {}))
...
class CPMNuker:
    ...
    async def _full_save_record(self, uid, data):
        """Exact full-record save request used by mainbotcpm.py."""
        ...
        try:
            ...
            payload = {"uid": fuid, "password": password, "email": email, "fk": FK, "base64": b64}
            r = await self._post(SAVE_URL, payload, {})
            if isinstance(r, dict) and isinstance(r.get("error"), dict):
                return {"ok": False, "message": f"FULL_SAVE_FAILED: {str(r)[:220]}"}
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock all clothes using CPM's native full-record save format."""
        await self.load(uid, force=True)
        ...
        current["personEquipmentsMale"] = [
            {"type": 0, "id": i, "color": 0} for i in range(1, 200)
        ]
        current["personEquipmentsFemale"] = [
            {"type": 0, "id": i, "color": 0} for i in range(1, 200)
        ]

        saved = await self._full_save_record(uid, current)
        if not saved.get("ok"):
            return saved
        ...
        fresh_td = self.get_token_data(uid) or {}
        fresh = self.get_record(uid, fresh_td.get("email", "")) or {}
        ...
        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            ...
            if not isinstance(items, list):
                return {
                    "ok": False,
                    "message": f"ALL_CLOTHES_VERIFY_FAILED:{key}"
                }
            ...
            for item in items:
                ...
                try:
                    if int(item.get("type", 0)) == 0:
                        owned.add(int(item.get("id", -1)))
                except (TypeError, ValueError):
                    continue

            missing = sorted(set(range(1, 200)) - owned)
            if missing:
                return {
                    "ok": False,
                    "message": (
                        f"ALL_CLOTHES_VERIFY_FAILED:{key}:"
