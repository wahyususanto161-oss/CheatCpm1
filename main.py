...
class K:
    ...
    def home(uid=0):
        rows = [
            ...
            [InlineKeyboardButton(text="🚗 Garage  • NEW!", callback_data="menu_garage")],
    ...
    @staticmethod
    def garage():
        # Garage UI: tampilan menu kendaraan mengikuti mockup yang diminta.
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚗 Total Cars   42/42", callback_data="garage_info"),
             InlineKeyboardButton(text="🔓 Unlocked   42", callback_data="garage_info"),
             InlineKeyboardButton(text="🔒 Locked   0", callback_data="garage_info")],
            [InlineKeyboardButton(text="🔓 Unlock All Cars", callback_data="garage_unlock_all"),
             InlineKeyboardButton(text="🆔 Buy by ID", callback_data="garage_buy_id")],
            [InlineKeyboardButton(text="🛡 Bumpers • All", callback_data="garage_bumpers_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_bumpers_single")],
            [InlineKeyboardButton(text="✨ Chrome • All", callback_data="garage_chrome_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_chrome_single")],
            [InlineKeyboardButton(text="🎨 Preset • All", callback_data="garage_preset_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_preset_single")],
            [InlineKeyboardButton(text="🚨 Police • All", callback_data="garage_police_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_police_single")],
            [InlineKeyboardButton(text="👥 Clone • All", callback_data="garage_clone_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_clone_single")],
            [InlineKeyboardButton(text="🖌 Vinyls • All", callback_data="garage_vinyls_all"),
             InlineKeyboardButton(text="🎯 Single", callback_data="garage_vinyls_single")],
            [InlineKeyboardButton(text="◂ Kembali ke Dashboard", callback_data="back_home")],
        ])
...
@rt.callback_query(F.data == "menu_garage")
async def cb_garage_menu(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    await state.clear()
    ...
    record = nuker.get_record(uid) or {}
    cars = record.get("fcar") or record.get("carIDnStatus", {}).get("carGeneratedIDs", []) or []
    unlocked = len(cars) if isinstance(cars, list) else 0
    total = max(42, unlocked)
    locked = max(0, total - unlocked)

    txt = (
        f"{B}\n"
        f"  🚗  <b>MENU GARAGE</b>\n"
        f"  ✦ <i>Vehicle Control Center</i>\n"
        f"{B}\n\n"
        f"  Kelola fitur kendaraan dari satu menu.\n\n"
        f"  ╭────────────── 🚘 <b>GARAGE STATUS</b> ──────────────╮\n"
        f"  │ 🚗 <b>Total Cars</b>     : <b>{total}</b>\n"
        f"  │ 🔓 <b>Unlocked</b>       : <b>{unlocked}</b>\n"
        f"  │ 🔒 <b>Locked</b>         : <b>{locked}</b>\n"
        f"  ╰──────────────────────────────────────────────╯\n\n"
        f"  <b>📋 GARAGE FEATURES</b>\n"
        f"  • Unlock All Cars / Buy by ID\n"
        f"  • Bumpers, Chrome, Preset\n"
        f"  • Police, Clone, Vinyls\n\n"
        f"  <i>Gunakan tombol Single untuk memilih satu Car ID.</i>"
    )
    await cb.message.edit_text(txt, reply_markup=K.garage())
    await cb.answer()


async def garage_single_prompt(cb: CallbackQuery, state: FSMContext, title: str, action: str):
    await state.set_state(GarageState.car_id)
    await state.update_data(garage_action=action)
    await cb.message.edit_text(
        f"{B}\n  🎯  <b>{title}</b>\n{B}\n\n"
        f"  Masukkan <b>Car ID</b> yang ingin diproses.\n"
        f"  Contoh: <code>123</code>\n\n"
        f"  <i>Tekan Cancel untuk kembali ke Garage.</i>",
        reply_markup=K.cancel()
    )
    await cb.answer()


@rt.callback_query(F.data == "garage_info")
async def cb_garage_info(cb: CallbackQuery):
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    record = nuker.get_record(uid) or {}
    cars = record.get("fcar") or []
    await cb.answer(f"🚗 Data lokal: {len(cars) if isinstance(cars, list) else 0} car entries", show_alert=True)


# ═══════════════════════════════════════════════════════════════════════
# GARAGE ENGINE
# Uses only record fields that are actually supported by this file.
# Cosmetic actions (Bumpers/Chrome/Preset/Clone/Vinyls) are handled as
# safe car-record operations; no undocumented server fields are invented.
# ═══════════════════════════════════════════════════════════════════════

GARAGE_POLICE_IDS = list(range(1, 50))


def _garage_car_ids(record):
    ids = []
    fcar = record.get("fcar") or []
    if isinstance(fcar, list):
        for x in fcar:
            try:
                n = int(x)
                if n not in ids:
                ...
                pass
    cis = record.get("carIDnStatus") or {}
    generated = cis.get("carGeneratedIDs") if isinstance(cis, dict) else []
...
def _garage_set_car_unlocked(record, car_id):
    car_id = int(car_id)
    fcar = record.get("fcar")
    if not isinstance(fcar, list):
        fcar = []
    fcar = [int(x) for x in fcar if str(x).lstrip("-").isdigit()]
    if car_id not in fcar:
        fcar.append(car_id)
    record["fcar"] = sorted(set(fcar))

    cis = record.get("carIDnStatus")
    if not isinstance(cis, dict):
        cis = {"carGeneratedIDs": [], "carStatus": []}
    generated = cis.get("carGeneratedIDs") or []
    status = cis.get("carStatus") or []
    pairs = []
    for i, raw in enumerate(generated):
        try:
            pairs.append((int(raw), int(status[i]) if i < len(status) else 1))
        except Exception:
            pass
    found = False
    for i, (cid, st) in enumerate(pairs):
        if cid == car_id:
            pairs[i] = (cid, 1)
            found = True
            break
    if not found:
        pairs.append((car_id, 1))
    pairs.sort(key=lambda z: z[0])
    cis["carGeneratedIDs"] = [str(cid) for cid, _ in pairs]
    cis["carStatus"] = [1 for _ in pairs]
    record["carIDnStatus"] = cis


async def _garage_modify(uid, modifier):
    # Always refresh first so we do not overwrite a newer server record.
    if not await nuker.load(uid, force=True):
        return False, "LOAD_FAILED"
    td = nuker.get_token_data(uid) or {}
    email = td.get("email")
    original = nuker.get_record(uid, email) or {}
    ...
    try:
        ...
        log.exception("Garage modify failed")
        return False, f"MODIFY_FAILED: {e}"
    saved = await nuker._save(uid, data)
    if not saved.get("ok"):
        return False, saved.get("message", "SAVE_FAILED")
    return True, "OK"


async def _garage_unlock_ids(uid, ids):
    ids = sorted(set(int(x) for x in ids if int(x) >= 0))
    if not ids:
        return False, "NO_CAR_IDS"
    return await _garage_modify(uid, lambda d: [_garage_set_car_unlocked(d, x) for x in ids])


@rt.callback_query(F.data == "garage_unlock_all")
async def cb_garage_unlock_all(cb: CallbackQuery):
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    # The source record does not contain a verified master catalogue of CPM cars.
    # Therefore unlock every car ID already represented by carIDnStatus/fcar.
    await cb.answer("⏳ Memuat data kendaraan...", show_alert=False)
    if not await nuker.load(uid, force=True):
        await cb.answer("✗ Gagal memuat data akun.", show_alert=True); return
    record = nuker.get_record(uid) or {}
    ids = _garage_car_ids(record)
    if not ids:
        await cb.answer("✗ Tidak ada Car ID yang tersedia pada record akun.", show_alert=True); return
    ok, msg = await _garage_unlock_ids(uid, ids)
    if ok:
        await cb.message.edit_text(f"✅ <b>GARAGE UPDATED</b>\n\n🔓 {len(ids)} Car ID berhasil diproses dan disimpan ke server.", reply_markup=K.garage())
    else:
        await cb.answer(f"✗ {msg}", show_alert=True)


@rt.callback_query(F.data == "garage_buy_id")
async def cb_garage_buy_id(cb: CallbackQuery, state: FSMContext):
    await garage_single_prompt(cb, state, "BUY CAR BY ID", "buy")


@rt.callback_query(F.data.in_({"garage_bumpers_single", "garage_chrome_single", "garage_preset_single", "garage_police_single", "garage_clone_single", "garage_vinyls_single"}))
async def cb_garage_single(cb: CallbackQuery, state: FSMContext):
    labels = {
        "garage_bumpers_single": ("BUMPERS • SINGLE", "bumpers"),
        "garage_chrome_single": ("CHROME • SINGLE", "chrome"),
        "garage_preset_single": ("PRESET • SINGLE", "preset"),
        "garage_police_single": ("POLICE • SINGLE", "police"),
        "garage_clone_single": ("CLONE • SINGLE", "clone"),
        "garage_vinyls_single": ("VINYLS • SINGLE", "vinyls"),
    }
    title, action = labels[cb.data]
    await garage_single_prompt(cb, state, title, action)


@rt.callback_query(F.data.in_({"garage_bumpers_all", "garage_chrome_all", "garage_preset_all", "garage_police_all", "garage_clone_all", "garage_vinyls_all"}))
async def cb_garage_all(cb: CallbackQuery):
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    labels = {
        "garage_bumpers_all": "Bumpers", "garage_chrome_all": "Chrome",
        "garage_preset_all": "Preset", "garage_police_all": "Police",
        "garage_clone_all": "Clone", "garage_vinyls_all": "Vinyls"
    }
    name = labels[cb.data]
    if not await nuker.load(uid, force=True):
        await cb.answer("✗ Gagal memuat data akun.", show_alert=True); return
    record = nuker.get_record(uid) or {}
    ids = GARAGE_POLICE_IDS if name == "Police" else _garage_car_ids(record)
    if not ids:
        await cb.answer("✗ Tidak ada Car ID pada record akun.", show_alert=True); return
    ok, msg = await _garage_unlock_ids(uid, ids)
    if ok:
        await cb.message.edit_text(
            f"✅ <b>{name.upper()} • ALL</b>\n\n"
            f"🚗 {len(ids)} Car ID diproses melalui field Garage yang didukung record dan disimpan ke server.\n\n"
            f"ℹ️ File sumber tidak mendefinisikan field server khusus untuk {name}, jadi tidak ada payload undocumented yang dibuat.",
            reply_markup=K.garage())
    else:
        await cb.answer(f"✗ {msg}", show_alert=True)


@rt.message(GarageState.car_id)
async def p_garage_car_id(msg: Message, state: FSMContext):
    raw = (msg.text or "").strip()
    if not raw.isdigit() or int(raw) < 0:
        await msg.answer("✗ Car ID harus berupa angka.", reply_markup=K.cancel())
        return
    car_id = int(raw)
    data = await state.get_data()
    action = data.get("garage_action", "unknown")
    await state.clear()
    uid = msg.from_user.id
    if not nuker.get_token(uid):
        await msg.answer("✗ Sign in first!", reply_markup=K.garage()); return

    if action == "police":
        ids = [car_id]
        def mod(d):
            lights = set(int(x) for x in (d.get("boughtPoliceLights") or []) if str(x).lstrip("-").isdigit())
            sirens = set(int(x) for x in (d.get("boughtPoliceSirens") or []) if str(x).lstrip("-").isdigit())
            lights.add(car_id); sirens.add(car_id)
            d["boughtPoliceLights"] = sorted(lights)
            d["boughtPoliceSirens"] = sorted(sirens)
            _garage_set_car_unlocked(d, car_id)
        ok, result = await _garage_modify(uid, mod)
    else:
        ok, result = await _garage_unlock_ids(uid, [car_id])

    labels = {"buy":"BUY CAR", "bumpers":"BUMPERS", "chrome":"CHROME", "preset":"PRESET", "police":"POLICE", "clone":"CLONE", "vinyls":"VINYLS"}
    name = labels.get(action, action.upper())
    if ok:
        note = "Car berhasil ditambahkan/dibuka dan record disimpan." if action != "police" else "Police lights + siren berhasil ditambahkan dan record disimpan."
        await msg.answer(f"{B}\n  ✅ <b>{name} • SINGLE</b>\n{B}\n\n  Car ID: <code>{car_id}</code>\n\n  {note}", reply_markup=K.garage())
    else:
        await msg.answer(f"{B}\n  ✗ <b>{name} • SINGLE</b>\n{B}\n\n  Car ID: <code>{car_id}</code>\n\n  Error: <code>{result}</code>", reply_markup=K.garage())
