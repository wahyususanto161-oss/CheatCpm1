...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock clothing IDs while preserving the user's existing inventory."""
        await self.load(uid)
        td = self.get_token_data(uid) or {}
        email = td.get("email")
        current = deepcopy(self.get_record(uid, email))
        ...
        # Use the same broad clothing range used by the feature implementation.
        clothing_ids = set(range(1, 301))

        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            equip = deepcopy(current.get(key) or {})

            # Keep the serialized equipment structure compatible with main.py.
            for part in clothing_parts:
                existing = equip.get(part) or []
                equip[part] = sorted(set(existing) | clothing_ids)

            equip.setdefault("SelectedEquipments", [])
            equip.setdefault("Gender", 0)
            current[key] = equip
        ...
        # Verify both gender inventories after a fresh server reload.
        if not await self.load(uid, force=True):
        ...
        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            ...
            if not isinstance(equip, dict):
                return {"ok": False, "message": f"ALL_CLOTHES_VERIFY_FAILED:{key}"}
            for part in clothing_parts:
                values = equip.get(part) or []
                try:
                    ids = {int(v) for v in values}
                except (TypeError, ValueError):
                    return {"ok": False, "message": f"ALL_CLOTHES_VERIFY_FAILED:{key}:{part}"}
