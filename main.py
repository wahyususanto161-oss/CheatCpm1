...
async def send_dashboard_card(target, caption, reply_markup=None):
    """Show the dashboard using the same configured photo as the welcome card."""
    photo = get_bot_photo()
    ...
    if isinstance(target, Message):
        ...
        try:
            return await target.edit_text(caption, reply_markup=reply_markup)
        except Exception:
            return await target.answer(caption, reply_markup=reply_markup)
    ...
    if isinstance(target, CallbackQuery):
        if photo:
            try:
                ...
                log.warning(f"Dashboard callback photo failed: {e}")
        try:
            return await target.message.edit_text(caption, reply_markup=reply_markup)
        except Exception:
            return None
...
async def show_home(target, uid):
    ...
    if not td:
        ...
        kb = K.home(uid)
        return await send_dashboard_card(target, txt, kb)
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if td:
        ...
        if record and record.get("Name"):
            ...
            await send_welcome(msg, T.welcome(name,un,uid), K.home(uid))
    else:
    ...
    try:
        ...
        if r.get("ok"):
            ...
            if loaded:
                ...
                await send_dashboard_card(ld, T.dashboard(record, em, uid), K.home(uid))
            else:
                await ld.edit_text(
                    f"{B}\n  ✅  𝗟𝗢𝗚𝗜𝗡 𝗦𝗨𝗖𝗖𝗘𝗦𝗦\n{B}\n\n  📧 {em}\n\n  ⚠ Tap Refresh to load data.",
...
async def cb_refresh(cb: CallbackQuery):
    ...
    record = nuker.get_record(uid, email)
    if record and record.get("Name"):
        await send_dashboard_card(cb, T.dashboard(record, email, uid), K.home(uid))
    else:
