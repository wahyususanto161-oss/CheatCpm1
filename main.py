...
class Reader:
    ...
    def read_equipment(self):
        """Read CPM's native flat equipment wire format."""
        count = self.read_int()
...
def serialize_full_record(record):
    ...
    w.write_equipment_flat(record.get("personEquipmentsMale") or [])
    w.write_equipment_flat(record.get("personEquipmentsFemale") or [])
...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock all clothes using the same record mechanism as mainbotcpm.py."""
        await self.load(uid, force=True)
        td = self.get_token_data(uid) or {}
        email = td.get("email", "")
        ...
        # mainbotcpm.py uses exactly this native equipment representation:
        # [{"type": 0, "id": i, "color": 0}] for i = 1..199.
        male = [{"type": 0, "id": i, "color": 0} for i in range(1, 200)]
        female = [{"type": 0, "id": i, "color": 0} for i in range(1, 200)]
        current["personEquipmentsMale"] = male
        current["personEquipmentsFemale"] = female

        # Use the normal authenticated partial-save path. It is the path
        # already used successfully by the other features in main.py and
        # serializes fields 41/42 in the native equipment wire format.
        saved = await self._save(uid, current)
        if not saved.get("ok"):
            return saved

        # Force a fresh server read. Never mark the feature as unlocked based
        # only on the local cache.
        if not await self.load(uid, force=True):
            return {"ok": False, "message": "ALL_CLOTHES_VERIFY_LOAD_FAILED"}

        fresh = self.get_record(uid, email) or {}
        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            items = fresh.get(key)
            if not isinstance(items, list):
                return {"ok": False, "message": "ALL_CLOTHES_VERIFY_FAILED"}
            owned = {
                int(item.get("id", -1))
                for item in items
                if isinstance(item, dict) and int(item.get("type", 0)) == 0
            }
            if not set(range(1, 200)).issubset(owned):
                return {"ok": False, "message": "ALL_CLOTHES_VERIFY_FAILED"}
