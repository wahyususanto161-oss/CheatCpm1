...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """Unlock clothing IDs 1..199 for both male and female inventories."""
        await self.load(uid)
        ...
        for key in ("personEquipmentsMale", "personEquipmentsFemale"):
            ...
            # Keep the exact equipment structure expected by main.py.
            for part in parts:
                current = equip.get(part) or []
                equip[part] = sorted(set(current) | clothing_ids)
        ...
        if not result.get("ok"):
            # Preserve the real server error so the Telegram UI can show it.
            return result
        return {"ok": True}
