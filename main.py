...
async def cb_garage_unlock_all(cb: CallbackQuery):
    if not nuker.get_token(cb.from_user.id):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    await cb.answer("🚗 Unlock All Cars belum terhubung ke endpoint game.", show_alert=True)
...
async def cb_garage_all(cb: CallbackQuery):
    ...
    name = labels[cb.data]
    await cb.answer(f"🚗 {name} • All belum terhubung ke endpoint game.", show_alert=True)
...
async def p_garage_car_id(msg: Message, state: FSMContext):
    ...
    name = labels.get(action, action.title())
    await msg.answer(
        f"{B}\n  🎯  <b>{name} • SINGLE</b>\n{B}\n\n"
        f"  Car ID: <code>{int(raw)}</code>\n\n"
        f"  ⚠️ <b>Belum dieksekusi.</b>\n"
        f"  Handler Garage sudah aktif, tetapi endpoint game untuk aksi ini belum tersedia di main2.py.\n\n"
        f"  Tidak ada klaim sukses atau perubahan akun palsu yang dibuat.",
