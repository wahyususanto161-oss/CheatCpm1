...
# Garage uses the same player-record endpoints as the account system.
...
class K:
    ...
    def garage():
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚗 Total Cars", callback_data="garage_info"),
             InlineKeyboardButton(text="🔓 Unlocked", callback_data="garage_info"),
             InlineKeyboardButton(text="🔒 Locked", callback_data="garage_info")],
...
async def cb_garage_menu(cb: CallbackQuery, state: FSMContext):
    ...
    generated = (record.get("carIDnStatus") or {}).get("carGeneratedIDs", [])
    ids = _garage_car_ids(record)
    unlocked = len(ids) if isinstance(ids, list) else 0
    ...
    txt = (
        ...
        f"  ╰──────────────────────────────────────────────╯\n\n"
        f"  <b>📋 PILIH FITUR GARAGE</b>\n"
        f"  • Unlock All Cars / Buy by ID\n"
        f"  • Bumpers, Chrome, Preset\n"
        f"  • Police, Clone, Vinyls\n\n"
        f"  <i>Car ID yang diproses berasal dari record akun.\n"
        f"  Field khusus kosmetik yang tidak ada di source tidak dibuat secara paksa.</i>"
    )
...
# ═══════════════════════════════════════════════════════════════════════
# GARAGE ENGINE
# Uses only fields already present in the player-record schema.
# The source does not define verified server fields for bumpers/chrome/
# preset/clone/vinyls, so those buttons do not invent undocumented payloads.
# ═══════════════════════════════════════════════════════════════════════
...
async def _garage_modify(uid, modifier):
    # Refresh before changing anything so we do not overwrite a newer record.
...
async def cb_garage_info(cb: CallbackQuery):
    ...
    record = nuker.get_record(uid) or {}
    ids = _garage_car_ids(record)
    await cb.answer(f"🚗 {len(ids)} Car ID tersimpan di record akun.", show_alert=True)
...
async def cb_garage_unlock_all(cb: CallbackQuery):
    ...
    if not await nuker.load(uid, force=True):
        await cb.answer("✗ Gagal memuat data akun.", show_alert=True)
        return
    record = nuker.get_record(uid) or {}
    ids = _garage_car_ids(record)
    if not ids:
        await cb.answer("✗ Tidak ada Car ID yang tersedia pada record akun.", show_alert=True)
        return
    ok, msg = await _garage_unlock_ids(uid, ids)
    if ok:
        await cb.message.edit_text(
            f"✅ <b>GARAGE UPDATED</b>\n\n"
            f"🔓 {len(ids)} Car ID diproses dan disimpan ke player record.",
...
@rt.callback_query(F.data.in_({
    "garage_bumpers_single", "garage_chrome_single", "garage_preset_single",
    "garage_police_single", "garage_clone_single", "garage_vinyls_single"
}))
...
@rt.callback_query(F.data.in_({
    "garage_bumpers_all", "garage_chrome_all", "garage_preset_all",
    "garage_police_all", "garage_clone_all", "garage_vinyls_all"
}))
async def cb_garage_all(cb: CallbackQuery):
    ...
    if not nuker.get_token(uid):
        ...
        return
    labels = {
        "garage_bumpers_all": "Bumpers",
        "garage_chrome_all": "Chrome",
        "garage_preset_all": "Preset",
        "garage_police_all": "Police",
        "garage_clone_all": "Clone",
        "garage_vinyls_all": "Vinyls",
    }
    name = labels[cb.data]
    if not await nuker.load(uid, force=True):
        await cb.answer("✗ Gagal memuat data akun.", show_alert=True)
    ...
    if not ids:
        await cb.answer("✗ Tidak ada Car ID pada record akun.", show_alert=True)
        return
    ...
    if name == "Police":
        ...
        ok, msg = await _garage_modify(uid, mod)
    else:
        ok, msg = await _garage_unlock_ids(uid, ids)

    if ok:
        detail = (
            "Police lights + sirens ditambahkan."
            if name == "Police"
            else "Car ID diproses menggunakan field player-record yang didukung source."
        )
        await cb.message.edit_text(
            f"✅ <b>{name.upper()} • ALL</b>\n\n"
            f"🚗 {len(ids)} Car ID diproses.\n"
            f"{detail}\n\n"
            f"ℹ️ Source tidak mendefinisikan field server khusus untuk {name}; tidak ada payload undocumented yang dibuat.",
            reply_markup=K.garage())
    else:
        await cb.answer(f"✗ {msg}", show_alert=True)
...
async def p_garage_car_id(msg: Message, state: FSMContext):
    ...
    uid = msg.from_user.id
    if not nuker.get_token(uid):
        await msg.answer("✗ Sign in first!", reply_markup=K.garage())
        return
    ...
    labels = {
        "buy":"BUY CAR", "bumpers":"BUMPERS", "chrome":"CHROME",
        "preset":"PRESET", "police":"POLICE", "clone":"CLONE", "vinyls":"VINYLS"
    }
    name = labels.get(action, action.upper())
    if ok:
        if action == "police":
            note = "Police lights + siren berhasil ditambahkan dan record disimpan."
        elif action == "buy":
            note = "Car berhasil ditambahkan/dibuka dan record disimpan."
        else:
            note = (
                "Car ID diproses melalui field player-record yang didukung source. "
                "Source tidak menyediakan field khusus untuk kosmetik ini."
            )
        await msg.answer(
            f"{B}\n  ✅ <b>{name} • SINGLE</b>\n{B}\n\n"
            f"  Car ID: <code>{car_id}</code>\n\n  {note}",
            reply_markup=K.garage())
    else:
        await msg.answer(
            f"{B}\n  ✗ <b>{name} • SINGLE</b>\n{B}\n\n"
            f"  Car ID: <code>{car_id}</code>\n\n"
            f"  Error: <code>{escape(str(result))}</code>",
            reply_markup=K.garage())
