...
class Reader:
    ...
    def read_equipment(self):
        """Read CPM equipment in the native flat-list wire format.

        Each equipment record is: int count, then repeated items of
        type 0/1/2/... exactly as used by mainbotcpm.py.
        """
...
class Writer:
    ...
    def write_equipment(self, data):
        """Write CPM equipment using the native flat-list wire format."""
        data = data or []
        if not isinstance(data, list):
            # Backward compatibility for older cached category-shaped data.
            data = equipment_to_native_flat(data)
...
class CPMNuker:
    ...
    async def unlock_all_clothes(self, uid):
        """All Clothes using the native full-record mechanism from mainbotcpm.py."""
        ...
        # mainbotcpm.py represents clothing as native flat equipment:
        # type=0, id=1..199, color=0 for both genders.
        native_items = [
            {"type": 0, "id": i, "color": 0}
            for i in range(1, 200)
        ]
        current["personEquipmentsMale"] = native_items
        current["personEquipmentsFemale"] = list(native_items)

        # Use the authenticated partial-save endpoint used by the other
        # working features. The previous full-record route returned
        # INVALID_ARGUMENT on this account/API. Clothes only need fields
        # 41/42, and _send() serializes those fields natively.
        ok, msg, auth = await self.get_auth(uid)
        if not ok:
            return {"ok": False, "message": msg}

        original = deepcopy(current)
        # Force the two equipment fields to be detected as modified.
        original["personEquipmentsMale"] = []
        original["personEquipmentsFemale"] = []
        ok2, msg2 = await self._send(
            auth, current, str((td or {}).get("firebase_uid", "")), original
        )
        if not ok2:
            return {"ok": False, "message": msg2}

        # Only report success after the server record can be loaded again.
        if not await self.load(uid, force=True):
            return {
                "ok": False,
