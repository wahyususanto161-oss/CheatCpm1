...
# ================= BOT TOKEN =================
BOT_TOKEN = "8689904423:AAEQS5lD1cBiSZe5nU1M57j22uC56_sY25I"
...
def _load_firebase_config():
    ...
    if config_file.exists():
        try:
            ...
            log.warning(f"Could not load firebase_config.json: {e}")
    ...
    if env_json:
        try:
            ...
            log.warning(f"Could not parse FIREBASE_SERVICE_ACCOUNT_JSON: {e}")
...
LOCAL_STORE_PATH = Path("local_bot_data.json")
...
def is_banned_fb(user_id):
    ...
    if time.time() - _banned_cache_time > 60:
        _executor.submit(_refresh_banned_cache)
...
def track_user(user):
    ...
    _executor.submit(fb_patch, f"users/{user_id}", data)

def log_user_action(user_id, action, details=""):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    log_entry = {"action": action, "details": details, "timestamp": now}
    _executor.submit(fb_patch, f"users/{user_id}/logs/{int(time.time()*1000)}", log_entry)
...
# ================= PAYMENT HANDLERS =================
async def notify_owner_payment(update, context, plan_name, stars, payment):
    """Notify the configured owner when a Telegram Stars purchase succeeds."""
    try:
        if not OWNER_ID:
            return

        user = update.effective_user
        username = f"@{user.username}" if user and user.username else "Tidak ada username"
        full_name = (user.full_name if user else "Unknown").replace("<", "&lt;").replace(">", "&gt;")
        user_id = user.id if user else "Unknown"

        telegram_charge_id = getattr(payment, "telegram_payment_charge_id", "-")
        ...
        message = (
            ...
            f"⭐ 𝙎𝙩𝙖𝙧𝙨 𝙋𝙖𝙞𝙙: <b>{stars}</b>\n"
            f"📦 𝙋𝙡𝙖𝙣: <b>{plan_name}</b>\n"
            "✅ 𝙎𝙩𝙖𝙩𝙪𝙨: <b>PAID</b>\n\n"
            "💎 Revenue: <b>BOT STAR BALANCE</b>\n"
            f"🧾 Telegram Charge ID: <code>{telegram_charge_id}</code>\n"
            f"🧾 Provider Charge ID: <code>{provider_charge_id}</code>"
        )
        ...
        log.warning(f"Could not notify owner about payment: {e}")
...
async def successful_payment_callback(update, context):
    ...
    await notify_owner_payment(update, context, plan_name, stars, payment)
