...
def get_text(chat_id, key, **kwargs):
    ...
    texts = {
        ...
        "free_trial_first": "🎁 **TRIAL MODE ACTIVE** // 10 MINUTES",
        "trial_activating": "⚡ **INITIALIZING TRIAL MODE...**",
        "start_normal_key": "🔑 KEY ACCESS",
        "start_time_key": "⏱️ TIME ACCESS",
        "start_free_trial": "🎁 TRIAL MODE",
...
def process_successful_stars_payment(message):
    ...
    try:
        ...
        expires_at = activate_vip(user_id, plan_code)
        save_star_payment(charge_id, user_id, plan_code, int(payment.total_amount), payment.currency)
        user_sessions.setdefault(chat_id, {})['logged_in'] = True
        user_sessions[chat_id]['is_vip'] = True
        user_sessions[chat_id]['vip_expires_at'] = expires_at
        total_users.add(chat_id)
        add_log(chat_id, f"VIP purchased via Telegram Stars: {plan_code}")
        ...
        notify_admins(
            ...
            f"📅 VIP Expires: `{format_vip_expiry(expires_at)}`\n"
        ...
        bot.send_message(
            ...
            f"📅 Active until: `{format_vip_expiry(expires_at)}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            "✅ VIP access has been activated automatically.",
            parse_mode='Markdown'
        )
        menu_command(message)
    except Exception as exc:
