...
# ═══════════════════════════════════════════════════════════
# 🧩 CUSTOM START MENU
# Extra buttons are managed by the owner/admin and stored locally.
# Each item: {"label": "...", "url": "..."}
# ═══════════════════════════════════════════════════════════
START_MENU_FILE = "start_menu_buttons.json"
DEFAULT_START_MENU_BUTTONS = [
    {"label": "📢 OFFICIAL CHANNEL", "url": CHANNEL_LINK},
]


def load_custom_start_buttons():
    try:
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    if isinstance(item, dict) and item.get("label") and item.get("url"):
                        clean.append({"label": str(item["label"])[:64], "url": str(item["url"]).strip()})
                return clean
    except Exception as exc:
        print(f"Start menu load error: {exc}")
    return []


def save_custom_start_buttons():
    try:
        with open(START_MENU_FILE, "w", encoding="utf-8") as f:
            json.dump(CUSTOM_START_BUTTONS, f, ensure_ascii=False, indent=2)
        return True
    except Exception as exc:
        print(f"Start menu save error: {exc}")
        return False


CUSTOM_START_BUTTONS = load_custom_start_buttons()
...
def init_stars_db():
    with sqlite3.connect(STARS_DB_PATH) as conn:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS vip_custom_plans (
            plan_code TEXT PRIMARY KEY,
...
def load_custom_vip_plans():
    """Load owner-created VIP durations so they appear in BUY VIP WITH STARS."""
    with sqlite3.connect(STARS_DB_PATH) as conn:
        rows = conn.execute("SELECT plan_code, days, stars, title FROM vip_custom_plans ORDER BY days ASC").fetchall()
    for plan_code, days, stars, title in rows:
        VIP_PLANS[str(plan_code)] = {
            "days": int(days),
            "stars": int(stars),
            "title": str(title),
            "custom": True,
...
def add_custom_vip_plan(days, stars, created_by):
    days = int(days)
    stars = int(stars)
    if days < 1 or days > 3650:
        raise ValueError("Days must be between 1 and 3650")
    if stars < 1 or stars > 250000:
        raise ValueError("Stars must be between 1 and 250000")

    plan_code = f"custom_{days}d"
    title = f"VIP {days} Day" if days == 1 else f"VIP {days} Days"
    with sqlite3.connect(STARS_DB_PATH) as conn:
        conn.execute("""INSERT INTO vip_custom_plans
                        (plan_code, days, stars, title, created_by, created_at)
                        VALUES (?, ?, ?, ?, ?, ?)
                        ON CONFLICT(plan_code) DO UPDATE SET
                        days=excluded.days, stars=excluded.stars, title=excluded.title,
                        created_by=excluded.created_by, created_at=excluded.created_at""",
                     (plan_code, days, stars, title, created_by, time.time()))
        conn.commit()
    VIP_PLANS[plan_code] = {"days": days, "stars": stars, "title": title, "custom": True}
    return plan_code


def get_vip_plans_sorted():
    return sorted(VIP_PLANS.items(), key=lambda item: (int(item[1].get("days", 0)), item[0]))
...
def set_vip_plan_price(plan_code, stars, updated_by):
    ...
    with sqlite3.connect(STARS_DB_PATH) as conn:
        if VIP_PLANS[plan_code].get("custom"):
            conn.execute("UPDATE vip_custom_plans SET stars=?, created_by=?, created_at=? WHERE plan_code=?",
                         (stars, updated_by, time.time(), plan_code))
...
def get_vip_prices_text():
    lines = ["⭐ **CURRENT VIP PRICES**", "━━━━━━━━━━━━━━━━━━━━━"]
    for _, plan in get_vip_plans_sorted():
        lines.append(f"💎 {plan['title']} : **{plan['stars']} Stars**")
    lines.append("━━━━━━━━━━━━━━━━━━━━━")
    lines.append("📌 Prices and custom VIP durations are saved permanently.")
    return "\n".join(lines)
...
def create_vip_keyboard():
    """Build the BUY VIP WITH STARS menu from standard + owner-created durations."""
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = []
    for plan_code, plan in get_vip_plans_sorted():
        days = int(plan["days"])
        day_label = "DAY" if days == 1 else "DAYS"
        buttons.append(types.InlineKeyboardButton(
            f"⭐ {days} {day_label} • {plan['stars']} Stars",
            callback_data=f"vip_buy_{plan_code}"
        ))
...
def create_start_keyboard(chat_id):
    """Build a larger start menu with owner-configurable extra buttons."""
    markup = types.InlineKeyboardMarkup(row_width=2)

    # Main access buttons stay side-by-side.
    markup.row(
        types.InlineKeyboardButton("🟣 KEY ACCESS", callback_data="normal_key"),
        types.InlineKeyboardButton("💎 FREE VIP KEY", callback_data="free_trial")
    )
    markup.row(types.InlineKeyboardButton("⭐ BUY VIP WITH STARS", callback_data="buy_vip"))

    # Extra default and custom buttons. Custom buttons can be added by admin.
    extras = DEFAULT_START_MENU_BUTTONS + CUSTOM_START_BUTTONS
    for i in range(0, len(extras), 2):
        row = []
        for item in extras[i:i + 2]:
            label = str(item.get("label", "🔗 LINK"))[:64]
            url = str(item.get("url", "")).strip()
            if url:
                row.append(types.InlineKeyboardButton(label, url=url))
        if row:
            markup.row(*row)
...
def create_vip_admin_keyboard(chat_id):
    ...
    buttons = [
        ...
        ("📅  ADD CUSTOM VIP DAYS", "admin_add_custom_vip_days"),
...
def handle_callback(call):
    ...
    # ====== Owner: VIP Prices & Custom VIP Durations ======
    if data == "admin_vip_prices":
        ...
        for plan_code, plan in get_vip_plans_sorted():
            buttons.append(types.InlineKeyboardButton(
                f"{plan['days']} DAYS • {plan['stars']} ⭐",
                callback_data=f"vip_price_{plan_code}"
            ))
        for i in range(0, len(buttons), 2):
            markup.row(*buttons[i:i + 2])
    ...
    if data == "admin_add_custom_vip_days":
        if not is_admin(chat_id):
            bot.answer_callback_query(call.id, "Owner only", show_alert=True)
            return
        user_states[chat_id] = {"awaiting_custom_vip_days": True}
        bot.send_message(
            chat_id,
            "📅 **ADD CUSTOM VIP DURATION**\n━━━━━━━━━━━━━━━━━━━━━\n"
            "Send the number of VIP days you want to add to **BUY VIP WITH STARS**.\n\n"
            "Example: `60`",
            parse_mode='Markdown'
        )
        bot.answer_callback_query(call.id)
    ...
    # ====== Admin: Custom Start Menu ======
    if data == "admin_start_menu":
        if not is_admin(chat_id):
            bot.answer_callback_query(call.id, "Admin only", show_alert=True)
            return
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.row(
            types.InlineKeyboardButton("➕ ADD BUTTON", callback_data="admin_start_add"),
            types.InlineKeyboardButton("📋 BUTTON LIST", callback_data="admin_start_list")
        )
        markup.row(
            types.InlineKeyboardButton("🗑 DELETE BUTTON", callback_data="admin_start_delete"),
            types.InlineKeyboardButton("♻️ RESET CUSTOM", callback_data="admin_start_reset")
        )
        markup.row(types.InlineKeyboardButton("🔙 BACK", callback_data="admin_panel"))
        bot.send_message(
            chat_id,
            "🧩 **CUSTOM START MENU**\n━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌 Default extra buttons: **{len(DEFAULT_START_MENU_BUTTONS)}**\n"
            f"➕ Custom buttons: **{len(CUSTOM_START_BUTTONS)}**\n\n"
            "Add as many custom URL buttons as you need.\n"
            "The buttons will appear on the `/start` menu.",
            reply_markup=markup, parse_mode='Markdown'
        )
        return
    ...
    if data == "admin_start_add":
        if not is_admin(chat_id):
            return
        user_states[chat_id] = {"awaiting_start_button_label": True}
        bot.send_message(chat_id, "➕ **ADD START BUTTON**\n━━━━━━━━━━━━━━━━━━━━━\nSend the button name.\nExample: `🔥 PREMIUM SHOP`", parse_mode='Markdown')
        return

    if data == "admin_start_list":
        if not is_admin(chat_id):
            return
        if CUSTOM_START_BUTTONS:
            text = "📋 **CUSTOM START BUTTONS**\n━━━━━━━━━━━━━━━━━━━━━\n"
            for i, item in enumerate(CUSTOM_START_BUTTONS, 1):
                text += f"{i}. **{item.get('label', '-')}**\n   🔗 `{item.get('url', '-')}`\n"
        else:
            text = "📋 **CUSTOM START BUTTONS**\n━━━━━━━━━━━━━━━━━━━━━\n📭 No custom buttons yet."
        bot.send_message(chat_id, text[:3900], parse_mode='Markdown')
        return

    if data == "admin_start_delete":
        if not is_admin(chat_id):
            return
        if not CUSTOM_START_BUTTONS:
            bot.send_message(chat_id, "📭 **No custom buttons to delete.**", parse_mode='Markdown')
            return
        text = "🗑 **DELETE START BUTTON**\n━━━━━━━━━━━━━━━━━━━━━\n"
        for i, item in enumerate(CUSTOM_START_BUTTONS, 1):
            text += f"{i}. {item.get('label', '-')}\n"
        text += "\nSend the button number to delete."
        user_states[chat_id] = {"awaiting_start_button_delete": True}
        bot.send_message(chat_id, text, parse_mode='Markdown')
        return

    if data == "admin_start_reset":
        if not is_admin(chat_id):
            return
        CUSTOM_START_BUTTONS.clear()
        save_custom_start_buttons()
        bot.send_message(chat_id, "♻️ **Custom start buttons have been reset.**", parse_mode='Markdown')
        return
...
def handle_all_messages(message):
    ...
    if chat_id in user_states:
        ...
        # ====== Admin: Custom Start Menu ======
        if state.get('awaiting_start_button_label'):
            if not is_admin(chat_id):
                user_states.pop(chat_id, None)
                return
            label = (text or '').strip()
            if not label:
                bot.send_message(chat_id, "❌ **Button name cannot be empty.**", parse_mode='Markdown')
                return
            user_states[chat_id] = {"awaiting_start_button_url": True, "start_button_label": label[:64]}
            bot.send_message(chat_id, "🔗 Now send the button URL.\nExample: `https://t.me/YourChannel`", parse_mode='Markdown')
            return

        if state.get('awaiting_start_button_url'):
            if not is_admin(chat_id):
                user_states.pop(chat_id, None)
                return
            url = (text or '').strip()
            if not (url.startswith('https://') or url.startswith('http://') or url.startswith('tg://')):
                bot.send_message(chat_id, "❌ **Invalid URL.** Use `https://...`, `http://...`, or `tg://...`", parse_mode='Markdown')
                return
            CUSTOM_START_BUTTONS.append({
                "label": state.get("start_button_label", "🔗 CUSTOM BUTTON"),
                "url": url
            })
            save_custom_start_buttons()
            user_states.pop(chat_id, None)
            bot.send_message(chat_id, "✅ **Start button added successfully!**\nIt will now appear in the `/start` menu.", parse_mode='Markdown')
            return

        if state.get('awaiting_start_button_delete'):
            if not is_admin(chat_id):
                user_states.pop(chat_id, None)
                return
            try:
                ...
                bot.send_message(chat_id, "❌ **Send a valid button number.**", parse_mode='Markdown')
                return
            if index < 0 or index >= len(CUSTOM_START_BUTTONS):
                bot.send_message(chat_id, "❌ **Button number not found.**", parse_mode='Markdown')
                return
            removed = CUSTOM_START_BUTTONS.pop(index)
            save_custom_start_buttons()
            user_states.pop(chat_id, None)
            bot.send_message(chat_id, f"🗑 **Deleted:** {removed.get('label', 'Custom Button')}", parse_mode='Markdown')
            return
        ...
        # ====== Owner: Add Custom VIP Days to Stars Store ======
        if state.get('awaiting_custom_vip_days'):
            if not is_admin(chat_id):
                del user_states[chat_id]
                return
            try:
                ...
                bot.send_message(chat_id, "❌ **Invalid duration.** Send a whole number between `1` and `3650` days.", parse_mode='Markdown')
                return
            user_states[chat_id] = {"awaiting_custom_vip_stars": True, "custom_vip_days": days}
            bot.send_message(
                chat_id,
                f"⭐ **SET PRICE FOR {days} DAYS**\n━━━━━━━━━━━━━━━━━━━━━\n"
                "Now send the price in Telegram Stars.\nExample: `100`",
                parse_mode='Markdown'
            )
            return

        if state.get('awaiting_custom_vip_stars'):
            if not is_admin(chat_id):
                del user_states[chat_id]
                return
            days = state.get('custom_vip_days')
            try:
                stars = int(text.strip())
                plan_code = add_custom_vip_plan(days, stars, chat_id)
                plan = VIP_PLANS[plan_code]
            except (ValueError, TypeError):
            ...
            bot.send_message(
                chat_id,
                f"✅ **CUSTOM VIP PACKAGE ADDED!**\n━━━━━━━━━━━━━━━━━━━━━\n"
                f"💎 Package: **{plan['title']}**\n"
                f"📅 Duration: **{plan['days']} Days**\n"
                f"⭐ Price: **{plan['stars']} Stars**\n\n"
                "📌 This package is now visible in **BUY VIP WITH STARS** and is saved permanently.",
                parse_mode='Markdown'
            )
            add_log(chat_id, f"Custom VIP Stars package added: {plan_code} ({plan['days']} days / {plan['stars']} Stars)")
            show_vip_admin_menu(chat_id)
            return
