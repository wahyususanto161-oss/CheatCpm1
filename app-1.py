...
def format_account_info(info: Dict[str, Any]) -> str:
    if not info.get("ok"):
        return "╭━━━ ⚠️ ACCOUNT STATUS ━━━╮\n│ ❌ **Unable to load account data**\n╰━━━━━━━━━━━━━━━━━━━━╯"
    return f"""
╭━━━ 👤 **ACCOUNT OVERVIEW** ━━━╮
│ 👤 **Name:** `{info.get('name', 'Unknown')}`
│ 🆔 **Player ID:** `{info.get('localID', 'Unknown')}`
...
def get_text(chat_id, key, **kwargs):
    ...
    texts = {
        "welcome": "╭━━━ 🚀 **𝗠𝗮𝘀𝗞𝘆𝘆𝗢𝗙𝗙𝗖 || 𝗕𝗼𝘁** ━━━╮\n│ ✦ **Next-Gen Control Center**\n│ ✦ Fast • Clean • Modern\n╰━━━━━━━━━━━━━━━━━━━━╯\n\n🔐 **ACCESS GATEWAY**\nChoose your access method below.",
        "cpm1_section": "╭━━━ 📱 **CPM1 CONTROL** ━━━╮\n│ ⚡ Advanced feature console\n╰━━━━━━━━━━━━━━━━━━━━╯",
        "cpm2_section": "╭━━━ 🎮 **CPM2 CONTROL** ━━━╮\n│ ⚡ Advanced feature console\n╰━━━━━━━━━━━━━━━━━━━━╯",
        "back": "🔙 Back",
...
def subscription_required(message):
    ...
    bot.send_message(chat_id, "❌ **You must subscribe to the channel first!**\n\n📢 **Channel:** [𝗠𝗮𝘀𝗞𝘆𝘆𝗢𝗙𝗙𝗖 || 𝗕𝗼𝘁](https://t.me/MasKyyOFFC)", reply_markup=markup, parse_mode='Markdown')
...
def create_start_keyboard(chat_id):
    ...
    markup.row(
        ...
        types.InlineKeyboardButton("⏱️  TIME KEY", callback_data="time_key")
    )
    markup.row(
        types.InlineKeyboardButton("💳  BUY KEY", callback_data="buy_key")
    )
    return markup
...
def create_main_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    if is_admin(chat_id):
        markup.row(types.InlineKeyboardButton("👑  ADMIN CONTROL", callback_data="admin_panel"))
    markup.row(types.InlineKeyboardButton("📱  CPM1", callback_data="section_cpm1"), types.InlineKeyboardButton("🎮  CPM2", callback_data="section_cpm2"))
    markup.row(types.InlineKeyboardButton("🚪  LOG OUT", callback_data="logout"))
    return markup

def create_cpm1_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = [("📧 Email","cpm1_change_email"),("🔐 Password","cpm1_change_pass"),("📋 Clone","cpm1_clone"),("🚗 Unlock Cars","cpm1_unlock_cars"),("⚡ W16","cpm1_w16"),("📯 Horns","cpm1_horns"),("⛽ Fuel","cpm1_fuel"),("🛡️ No Damage","cpm1_damage"),("💨 Smoke","cpm1_smoke"),("👑 King Rank","cpm1_rank_advanced"),("🔧 Fix Account","cpm1_fix"),("🆔 Change ID","cpm1_change_id"),("💰 Money","cpm1_money"),("💎 Coins","cpm1_coin"),("🎭 Animations","cpm1_unlock_animations"),("🛞 Wheels","cpm1_unlock_wheels"),("🏠 Houses","cpm1_unlock_houses"),("🏆 Levels","cpm1_complete_levels"),("👨 Male Equip","cpm1_unlock_equip_male"),("👩 Female Equip","cpm1_unlock_equip_female")]
    for i in range(0,len(buttons),2):
        markup.row(*[types.InlineKeyboardButton(label, callback_data=data) for label,data in buttons[i:i+2]])
    markup.row(types.InlineKeyboardButton("💀  ULTIMATE UNLOCK", callback_data="cpm1_ultimate"))
    markup.row(types.InlineKeyboardButton("↻  REFRESH ACCOUNT", callback_data="refresh_account"))
    markup.row(types.InlineKeyboardButton("◀️  BACK TO MAIN", callback_data="back_main"))
    return markup

def create_cpm2_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.row(types.InlineKeyboardButton("👑  KING RANK", callback_data="cpm2_king_rank"))
    markup.row(types.InlineKeyboardButton("🎲  GENERATE ACCOUNT", callback_data="cpm2_generate"))
    markup.row(types.InlineKeyboardButton("◀️  BACK TO MAIN", callback_data="back_main"))
    return markup

def create_unlock_cars_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(types.InlineKeyboardButton("🖐️  MANUAL", callback_data="unlock_manual"), types.InlineKeyboardButton("🤖  AUTO • 1–270", callback_data="unlock_auto"))
    markup.row(types.InlineKeyboardButton("◀️  BACK", callback_data="back_cpm1"))
    return markup

def create_unlock_auto_confirm_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(types.InlineKeyboardButton("✅  CONFIRM", callback_data="unlock_auto_confirm"), types.InlineKeyboardButton("✖️  CANCEL", callback_data="unlock_auto_cancel"))
    return markup

def create_admin_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = [("📊 Stats","admin_stats"),("📢 Broadcast","admin_broadcast"),("🔑 Keys","admin_keys"),("⏱️ Time Keys","admin_time_keys"),("📈 Key Stats","admin_key_stats"),("👥 Key Users","admin_key_users"),("↻ Refresh All","admin_refresh_all"),("🚫 Ban","admin_ban"),("✅ Unban","admin_unban"),("📝 Logs","admin_logs"),("💾 Accounts","admin_saved"),("⚙️ Status","admin_status")]
    for i in range(0,len(buttons),2):
        markup.row(*[types.InlineKeyboardButton(label, callback_data=data) for label,data in buttons[i:i+2]])
    markup.row(types.InlineKeyboardButton("◀️  BACK TO MAIN", callback_data="back_main"))
    return markup
...
def menu_command(message):
    ...
    if chat_id not in user_sessions or not user_sessions[chat_id].get('logged_in'):
        ...
        return
    bot.send_message(chat_id, "╭━━━ 🚀 **CPM CONTROL CENTER** ━━━╮\n│ 📱 **CPM1**  • Advanced controls\n│ 🎮 **CPM2**  • Account utilities\n╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n\n💡 Select a module to continue.", reply_markup=create_main_keyboard(chat_id), parse_mode='Markdown')
...
def handle_callback(call):
    ...
    if data not in ["check_sub", "normal_key", "time_key", "free_trial", "buy_key"]:
        if not check_subscription(chat_id):
    ...
    # ====== Buy Key ======
    if data == "buy_key":
        bot.answer_callback_query(call.id, "💳 Opening key purchase...")
        owner_url = "tg://user?id=8700382637"
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton("👑 CONTACT OWNER", url=owner_url)
        )
        bot.send_message(
            chat_id,
            "💳 **BUY KEY**\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "🔑 Need a key to access the bot?\n"
            "📩 Contact the owner to purchase your key.\n\n"
            "👑 **Owner:** Admin / Owner\n"
