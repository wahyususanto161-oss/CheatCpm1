...
def create_admin_keyboard(chat_id):
    ...
    buttons = [
        ...
        ("📜 STAR HISTORY", "admin_star_history"),
    ]
...
def section_cpm1(message):
    ...
    bot.send_message(
        ...
        "│        📱 **CPM1 LOGIN**             │\n"
        "╠══════════════════════════════╣\n"
        "│ Masuk untuk membuka Control Panel.   │\n"
        "╚══════════════════════════════╝\n\n"
        "📧 **Enter CPM1 email:**",
        parse_mode='Markdown'
    )
...
def show_vip_search_results(chat_id, query):
    ...
    text += f"🔍 Query: `{query}`\n"
    if not results:
        text += "\n📭 VIP user atau VIP Key tidak ditemukan."
    else:
...
def buy_vip_command(message):
    ...
    bot.send_message(
        chat_id,
        "⭐ **MASKYY VIP STORE**\n━━━━━━━━━━━━━━━━━━━━━\n"
        "Pilih durasi VIP. Pembayaran menggunakan Telegram Stars dan VIP aktif otomatis setelah pembayaran berhasil.",
...
def stars_command(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            "Gunakan data ini sebagai saldo/riwayat resmi yang dikembalikan Telegram Bot API.",
            parse_mode='Markdown'
        )
    except Exception as exc:
...
def set_vip_key_lock(key, locked=True):
    key = key.strip().upper()
    if key not in TRIAL_KEYS: return False
    TRIAL_KEYS[key]["locked"] = bool(locked)
    return True
...
def handle_callback(call):
    ...
    if data == "buy_vip":
        ...
        bot.send_message(
            chat_id,
            "⭐ **MASKYY VIP STORE**\n━━━━━━━━━━━━━━━━━━━━━\nPilih paket VIP yang ingin dibeli:",
    ...
    if data == "admin_delete_vip_key":
        ...
        bot.send_message(chat_id, "🗑 **DELETE VIP KEY**\n━━━━━━━━━━━━━━━━━━━━━\nKirim VIP Key yang ingin dihapus.", parse_mode='Markdown')
        bot.answer_callback_query(call.id)
        return
    ...
    if data == "admin_vip_search":
        ...
        bot.send_message(chat_id, "🔎 **VIP SEARCH**\n━━━━━━━━━━━━━━━━━━━━━\nKirim **User ID** atau **VIP Key** untuk dicari.", parse_mode='Markdown')
        bot.answer_callback_query(call.id)
        return
    ...
    if data == "admin_reset_vip":
        ...
        bot.send_message(chat_id, "♻️ **RESET VIP USER**\n━━━━━━━━━━━━━━━━━━━━━\nKirim **User ID** yang VIP-nya ingin dicabut/reset.\n\n⚠️ Key yang sudah digunakan tidak akan dibuat ulang.", parse_mode='Markdown')
        bot.answer_callback_query(call.id)
        return
    ...
    if data.startswith("vip_key_unit_"):
        ...
        bot.send_message(
            chat_id,
            f"💎 **CUSTOM VIP KEY**\n━━━━━━━━━━━━━━━━━━━━━\n"
            f"Kirim jumlah **{unit_name}** untuk VIP Key.\n\nContoh: `30`",
            parse_mode='Markdown'
        )
        bot.answer_callback_query(call.id)
        return
...
def handle_all_messages(message):
    ...
    if chat_id in user_states:
        ...
        if state.get('awaiting_vip_lock') or state.get('awaiting_vip_unlock'):
            ...
            if set_vip_key_lock(key, locked):
                status = "🔒 LOCKED" if locked else "🔓 UNLOCKED"
                bot.send_message(chat_id, f"{status}\n━━━━━━━━━━━━━━━━━━━━━\n🔑 `{key}`", parse_mode='Markdown')
                add_log(chat_id, f"VIP key {'locked' if locked else 'unlocked'}: {key}")
            else:
                bot.send_message(chat_id, "❌ **VIP Key tidak ditemukan.**", parse_mode='Markdown')
        ...
        if state.get('awaiting_reset_vip'):
            ...
            if ok:
                ...
                bot.send_message(chat_id, "❌ **User tersebut tidak memiliki data VIP.**", parse_mode='Markdown')
            else:
                bot.send_message(chat_id, "❌ **User ID tidak valid.**", parse_mode='Markdown')
            del user_states[chat_id]
            admin_panel(message)
            return
        ...
        if state.get('awaiting_delete_vip_key'):
            ...
            ok, result = delete_vip_key(key)
            if ok:
                bot.send_message(chat_id, f"🗑 **VIP KEY DELETED**\n━━━━━━━━━━━━━━━━━━━━━\n🔑 `{key}`", parse_mode='Markdown')
                add_log(chat_id, f"VIP key deleted: {key}")
            else:
                bot.send_message(chat_id, "❌ **VIP Key tidak ditemukan.**", parse_mode='Markdown')
            del user_states[chat_id]
            admin_panel(message)
        ...
        if state.get('awaiting_extend_vip_key'):
            ...
            try:
                ...
                if not ok:
                    bot.send_message(chat_id, "❌ **VIP Key tidak ditemukan atau data tidak valid.**", parse_mode='Markdown')
                else:
                    ...
                    if new_expiry:
                        ...
                        msg += "\n📌 Tambahan durasi akan aktif saat key digunakan."
                    bot.send_message(chat_id, msg, parse_mode='Markdown')
                    add_log(chat_id, f"VIP key extended: {key} + {label}")
            except Exception:
                bot.send_message(chat_id, "❌ **Format salah.**\nGunakan: `VIPKEY | jumlah | minutes/hours/days`", parse_mode='Markdown')
                return
            del user_states[chat_id]
            admin_panel(message)
            return
        ...
        if state.get('awaiting_vip_key_duration'):
            ...
            try:
                ...
                bot.send_message(chat_id, "❌ **Masukkan angka yang valid dan lebih dari 0.**", parse_mode='Markdown')
                return
            del user_states[chat_id]
            admin_panel(message)
            return
